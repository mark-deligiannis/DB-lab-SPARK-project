...
spark = SparkSession \
    .builder \
    .appName("DF query 4 multiple joins execution") \
    .getOrCreate()
...
# Function to use different joins
def join_custom(lapd_ps, crime_df, join_type):
        # First Query
        ...
        crime_df1 = crime_df.join(lapd_ps.hint(join_type), ['AREA']) \
        ...
        crime_df2 = crime_df.join(lapd_ps.hint(join_type)) \
                            .withColumn("Distance", dist_udf(col("LAT"), col("LON"), col("Y"), col("X"))) \
                            .drop("LAT", "LON", "Y", "X")
...
print("\n==========Broadcast Join==========\n")
start = time.time()
join_custom(lapd_ps, crime_df, "broadcast")
print("Time needed:", time.time() - start)
print("\n\n==========Merge Join==========\n")
start = time.time()
join_custom(lapd_ps, crime_df, "merge")
print("Time needed:", time.time() - start)
print("\n\n==========Shuffle Hash Join==========\n")
start = time.time()
join_custom(lapd_ps, crime_df, "shuffle_hash")
print("Time needed:", time.time() - start)
print("\n\n==========Shuffle Replicate NL Join==========\n")
start = time.time()
join_custom(lapd_ps, crime_df, "shuffle_replicate_nl")
print("Time needed:", time.time() - start)
print("\n")
